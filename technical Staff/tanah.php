           ...
           <div id="" class="muted pull-right">
			...
			$my_device = mysql_query("select * from location_details    
             ...
             (select * from location_details where dev_status='Dump')
			  and dev_name = 'TANAH'  and stdev_id = '$get_id'
			  OR NOT EXISTS 
             (select * from location_details where dev_status='Dump')
			  and dev_name = 'Tanah'    and stdev_id = '$get_id'
			  OR NOT EXISTS 
             (select * from location_details where dev_status='Dump')
			  ...
			  OR NOT EXISTS 
             (select * from location_details where dev_status='Dump')
			  and dev_name = 'tnah'      and stdev_id = '$get_id'
			  OR NOT EXISTS 
             (select * from location_details where dev_status='Dump')
			  and dev_name = 'nah'      and stdev_id = '$get_id' 
			  OR NOT EXISTS 
             (select * from location_details where dev_status='Dump')
			  and dev_name = 'thna'       and stdev_id = '$get_id' 
			  OR NOT EXISTS 
             (select * from location_details where dev_status='Dump')
			  and dev_name = 'hanat'      and stdev_id = '$get_id' 
			  OR NOT EXISTS 
             (select * from location_details where dev_status='Dump')
			  and dev_name = 'tnh'  and stdev_id = '$get_id' 
			  OR NOT EXISTS 
             (select * from location_details where dev_status='Dump')
			  and dev_name = '(tanah)'    and stdev_id = '$get_id' ")or die(mysql_error());
			$count_my_device = mysql_num_rows($my_device);?>
			Daftar tanah: <span class="badge badge-info"><?php echo $count_my_device; ?></span>
			</div>
								
        </div>
							
<!-----------------------------------for Print display visible------------------------------------>	
<h4 id="sc">Location:<?php echo $location_row['stdev_location_name']; ?> Daftar tanah
 ...
 <br/>
  <!-----------------------------------device categorized by their location details using nav pills------------------------------------>	
	<div class="pull-left">
			...
			<?php	
	          $tanah = mysql_query("select * from location_details    
	          LEFT JOIN stdevice ON location_details.id = stdevice.id
			  LEFT JOIN device_name ON stdevice.dev_id=device_name.dev_id
			  where NOT EXISTS 
             (select * from location_details where dev_status='Dump')
             ...
             (select * from location_details where dev_status='Dump')
			  and dev_name = '(tanah)'    and stdev_id = '$get_id'")or die(mysql_error());
			  $count = mysql_num_rows($tanah);
			  ?>
              					
				<li class="active">			
				    <a href= 'tanah.php<?php echo '?stdev_id='.$get_id; ?>')">tanah&nbsp;<span class="label label-default"> <?php echo $count;?></span></a>
				</li>
